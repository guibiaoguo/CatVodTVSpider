...
public class Jumi extends Spider {
    ...
    private static final String siteUrl = "https://jumi.one";
    private static final String siteHost = "jumi.one";
    ...
    public String homeContent(boolean filter) {
        try {
            ...
            Elements elements = doc.select("ul.navbar-nav>li>a");
            ...
            try {
                // 取首页推荐视频列表
                Elements list = doc.select(".col-md-8 li");
                JSONArray videos = new JSONArray();
                for (int i = 0; i < list.size(); i++) {
                    Element vod = list.get(i);
                    String title = vod.select(".title").text();
                    String cover = vod.select(".video-pic").attr("data-original");
                    String remark = vod.select(".subtitle").text();
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) {
        try {
            ...
            // 取页码相关信息
            Elements pageInfo = doc.select(".box-page li a");
            if (pageInfo.size() == 0) {
                ...
                for (int i = 0; i < pageInfo.size(); i++) {
                    ...
                    if (name.equals("尾页") || name.equals("尾頁")) {
                        Matcher matcher = regexPage.matcher(a.attr("href"));
                        if (matcher.find()) {
            ...
            if (!html.contains("没有找到您想要的结果哦")) {
                // 取当前分类页的视频列表
                Elements list = doc.select(".box-video-list li");
                for (int i = 0; i < list.size(); i++) {
                    Element vod = list.get(i);
                    String title = vod.select(".title").text();
                    String cover = vod.select(".video-pic").attr("data-original");
    ...
    public String detailContent(List<String> ids) {
        try {
            ...
            // 取基本数据
            String vid = doc.select("span.mac_hits").attr("data-id");

            String cover = doc.select(".video-pic").attr("style");
            String title = doc.select(".details-info h1").text();
            String desc = Jsoup.parse(doc.select("meta[name=description]").attr("content")).text();
            String category = "", area = "", year = "", remark = "", director = "", actor = "";
            Elements span_text_muted = doc.select(".details-info li");
            for (int i = 0; i < span_text_muted.size(); i++) {
                Element text = span_text_muted.get(i);
                String info = text.text();
                if (info.equals("分类：") || info.equals("類別：")) {
                    category = text.nextElementSibling().text();
                } else if (info.equals("年份：") || info.equals("年代：")) {
                    year = text.nextElementSibling().text();
                } else if (info.equals("地区：") || info.contains("地區")) {
            ...
            // 取播放列表数据
            Elements sources = doc.select(".playlist .gico");
            Elements sourceList = doc.select(".playlist");

            for (int i = 0; i < sources.size(); i++) {
                Element source = sources.get(i);
                String sourceName = source.text();
                String playList = "";
                Elements playListA = sourceList.get(i).select("li:not(:has(a.gico)) a");
    ...
    public String playerContent(String flag, String id, List<String> vipFlags) {
        ...
        return Result.get().url(siteUrl + "/play/" + id + ".html").parse().string();
    }
