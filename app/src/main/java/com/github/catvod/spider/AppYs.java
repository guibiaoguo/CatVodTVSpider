    ...
    private void genPlayList(String URL, JSONObject object, String json, JSONObject vod, String vid) throws JSONException {
        ...
        if (URL.contains("api.php/app/")) {
            ...
            for (int i = 0; i < vodUrlWithPlayer.length(); i++) {
                ...
                String flag = stopVipFlag(from.optString("name"));
                playFlags.add(flag);
            ...
            for (int i = 0; i < vodUrlWithPlayer.length(); i++) {
                JSONObject from = vodUrlWithPlayer.optJSONObject(i);
                String flag = stopVipFlag(from.optJSONObject("player_info").optString("show"));
                playFlags.add(flag);
                playUrls.add(from.optString("url"));
                try {
    ...
    private void getFinalVideo(String uu, JSONObject result) throws JSONException {
        if (uu.contains("baidu.com")) {
            ...
            if (playurl.contains("duoduozy.com") || playurl.contains("suoyo.cc")) {
                ...
                result.put("parse", 0);
                result.put("playUrl", "");
                result.put("url", obj.optString("url"));
                ...
                if (playurl.contains("html")) {
                    ...
                    JSONObject obj = new JSONObject(content);
                    String ppurl = obj.optString("url");
                    result.put("parse", 0);
                    result.put("playUrl", "");
