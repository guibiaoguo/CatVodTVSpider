...
public class IQIYI extends Spider {
    ...
    public void init(Context context) {
        super.init(context);
        classifyMap.put("1", "电影");
        classifyMap.put("2", "连续剧");
        classifyMap.put("3", "纪录片");
        classifyMap.put("6", "综艺");
        classifyMap.put("4", "动漫");
    }
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) {
        try {
            String url = siteUrl + "/search/video/videolists?channel_id="+tid+"&is_purchase=&mode=24&pageNum="+pg+"&pageSize=24&data_type=1&site=iqiyi";
            SpiderUrl su = new SpiderUrl(url, getHeaders(url));
            SpiderReqResult srr = SpiderReq.get(su);
            JSONObject result = new JSONObject();
            try {
    ...
    public String detailContent(List<String> ids) {
        try {
            ...
            vodList.put("vod_name",vod.optString("shortTitle").replaceAll("第\\d+(?:集|期)",""));
