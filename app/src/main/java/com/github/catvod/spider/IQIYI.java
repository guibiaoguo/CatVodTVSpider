    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) {
        try {
            ...
            if (extend != null) {
                ...
                List<String> three_category_id = new ArrayList<>();
                for (String key : keys) {
                    String val = extend.get(key).trim();
                    if (key.matches("\\d+")) {
                        three_category_id.add(val + ";must");
                    } else {
                        url += "&" + key + "=" + val;
                    }
                }
