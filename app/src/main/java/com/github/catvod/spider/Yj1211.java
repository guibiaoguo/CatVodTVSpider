...
public class Yj1211 extends Spider {
    ...
    public String homeContent(boolean filter) {
        try {
            ...
            while (it.hasNext()) {
                ...
                jsonObject.put("type_id", catedef.optString(key));
            ...
            for (int j = 0; j < aidoa.length(); j++) {
                JSONObject newTypeExtend = new JSONObject();
                String typeName = aidoa.getJSONArray(j).getJSONObject(0).optString("typeName");
                ...
                for (int k = 0; k < fg; k++) {
                    kv = new JSONObject();
                    String areaName = aidoa.getJSONArray(j).getJSONObject(k).optString("areaName");
                    kv.put("n", areaName);
            ...
            for (int i = 0; i < 5; i++) {
                String typeId = classes.getJSONObject(i).optString("type_id");
            ...
            for (int i = 0; i < ch; i++) {
                ...
                String platForm = srcoria.getJSONObject(i).optString("platForm");
                String rd = srcoria.getJSONObject(i).optString("roomId");
                String id = platForm + "&" + rd;
                String name = srcoria.getJSONObject(i).optString("ownerName");
                String pic = srcoria.getJSONObject(i).optString("ownerHeadPic");
                String mark = srcoria.getJSONObject(i).optString("categoryName");
    ...
    public String detailContent(List<String> ids) {
        try {
            ...
            String title = srcdtlo.optString("roomName");
            String pic = srcdtlo.optString("roomPic");
            String director = srcdtlo.optString("ownerName") + " RoomID:" + srcdtlo.optString("roomId");
            String content = srcdtlo.optString("categoryName");
            String actor = "观看人数:" + srcdtlo.optString("online");
            String area = srcdtlo.optString("platForm");
        ...
        try {
            ...
            if (sra.length() > 0) {
                ...
                for (int i = 0; i < ch; i++) {
                    ...
                    String platForm = sra.getJSONObject(i).optString("platform");
                    String rd = sra.getJSONObject(i).optString("roomId");
                    String id = platForm + "&" + rd;
                    String name = sra.getJSONObject(i).optString("nickName");
                    String pic = sra.getJSONObject(i).optString("headPic");
                    String mark = "";
                    if (!sra.getJSONObject(i).isNull("cateName")) {
                        mark = sra.getJSONObject(i).optString("cateName");
                    }
